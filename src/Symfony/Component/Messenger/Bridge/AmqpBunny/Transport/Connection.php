...
class Connection
{
        ...
        if (null === $this->client) {
            ...
            if (!$this->client->isConnected()) {
                try {
                    ...
                    $credentials = $this->configuration['client'];
                    $credentials['password'] = '********';

                    throw new TransportException(sprintf('Could not connect to the AMQP server. Please verify the provided DSN. (%s).', json_encode($credentials, \JSON_UNESCAPED_SLASHES)), 0, $e);
                }
            }
        }
    ...
    private function declareDelayQueue(int $delay, string $routingKey, bool $isRetryAttempt)
    {
        ...
        $arguments = [
            'x-message-ttl' => $delay,
            // Auto-delete queue. If another message gets published into this queue, lease will be renewed
