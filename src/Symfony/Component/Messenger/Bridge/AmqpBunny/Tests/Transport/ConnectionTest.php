...
use Symfony\Component\Messenger\Bridge\AmqpBunny\Transport\BunnyFactory;
use Symfony\Component\Messenger\Bridge\AmqpBunny\Transport\BunnyStamp;
...
class ConnectionTest extends TestCase
{
    ...
    public function testItObfuscatesPasswordInDsnWhenConnectionFailed()
    {
        $this->expectException(TransportException::class);
        $this->expectExceptionMessage('Could not connect to the AMQP server. Please verify the provided DSN. ({"host":"localhost","port":5672,"user":"user","password":"********","vhost":"/"})');

        $this->client->expects($this->once())->method('isConnected')->willReturn(false);
        $this->client->expects($this->once())->method('connect')->willThrowException(new ClientException('Whoopsie'));

        $connection = Connection::fromDsn('amqp+bunny://user:secretpassword@localhost', [], $this->factory);
    ...
    public function testItRunsAutoSetupOnceForGet()
    {
        $this->channel->expects($this->exactly(2))->method('exchangeDeclare');
        $this->channel->expects($this->exactly(1))->method('queueDeclare');
        $this->channel->expects($this->exactly(1))->method('queueBind');
    ...
    public function testItRunsAutoSetupOnceForPublish()
    {
        $this->channel->expects($this->exactly(2))->method('exchangeDeclare');
        $this->channel->expects($this->exactly(1))->method('queueDeclare');
        $this->channel->expects($this->exactly(1))->method('queueBind');
    ...
    public function testItRunsAutoSetupOnceForCountMessagesInQueues()
    {
        $this->channel->expects($this->exactly(2))->method('exchangeDeclare');
        // Two calls because count does passive queue declaration
        $this->channel->expects($this->exactly(2))->method('queueDeclare')->willReturn((object) ['messageCount' => 0]);
        $this->channel->expects($this->exactly(1))->method('queueBind');
